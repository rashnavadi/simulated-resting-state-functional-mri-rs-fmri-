function [x, y, z, corr_over_time_xy, corr_over_time_xz, corr_over_time_yz] = generateCorrelatedSinusoids_2flips(nTimePts, transitionPoint1, transitionPoint2)
    ...
    y = zeros(size(x));
    y(1:transitionPoint1) = sin(2 * pi * (t(1:transitionPoint1) * 0.06) + phi_xy_low);
    y(transitionPoint1+1:transitionPoint2) = sin(2 * pi * (t(transitionPoint1+1:transitionPoint2) * 0.06) + phi_xy_high);
    y(transitionPoint2+1:end) = sin(2 * pi * (t(transitionPoint2+1:end) * 0.06) + phi_xy_low);

    % Generate the third timeseries (z, another sine wave with changing correlation to y)
    z = zeros(size(x));
    z(1:transitionPoint1) = sin(2 * pi * (t(1:transitionPoint1) * 0.06) + acos(desired_corr_xz));
    z(transitionPoint1+1:transitionPoint2) = sin(2 * pi * (t(transitionPoint1+1:transitionPoint2) * 0.06) + acos(desired_corr_yz_high));
